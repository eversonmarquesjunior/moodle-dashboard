...
${d.curador && d.insersor ? _chatBtn(d.id) : ''}
          ${moodleBtn}
          ${teamsBtn}
          <button data-edit="${esc(d.id)}"
            class="flex items-center gap-1.5 text-xs font-semibold
                ...
                border border-slate-200 hover:border-blue-300
                px-3 py-1.5 rounded-lg transition-all shadow-sm">
            ${_iconEdit()} Editar Card
          </button>
          <button data-move="${esc(d.id)}"
            class="flex items-center gap-1.5 text-xs font-semibold
                   bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white
                   px-3 py-1.5 rounded-lg transition-all duration-200 hover:scale-105 hover:shadow-md
                   active:scale-95 active:shadow-sm shadow-sm whitespace-nowrap">
...
function _chatBtn(id) {
  const hasUnread = typeof _unreadDisciplines !== 'undefined' && _unreadDisciplines.has(String(id));
  return `<button data-chat="${esc(id)}"
            class="relative flex items-center gap-1.5 text-xs font-semibold
                   bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white
                   px-3 py-1.5 rounded-lg transition-all duration-200 hover:scale-105 hover:shadow-md
                   active:scale-95 active:shadow-sm shadow-sm whitespace-nowrap">
            ${_iconChat()} Chat
            <span class="chat-unread-dot absolute -top-1.5 -right-1.5 w-3 h-3 bg-red-500 rounded-full border-2 border-white${hasUnread ? '' : ' hidden'}"></span>
          </button>`;
}
