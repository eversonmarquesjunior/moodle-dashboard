...
-- ── 6. Tabela de chats (nova funcionalidade) ─────────────────
CREATE TABLE IF NOT EXISTS public.chats (
  id            UUID         DEFAULT gen_random_uuid() PRIMARY KEY,
  discipline_id TEXT         NOT NULL REFERENCES public.disciplinas(id) ON DELETE CASCADE,
  sender        TEXT         NOT NULL,  -- e.g. 'FELIPE', 'JUNIOR'
  message       TEXT         NOT NULL,
  created_at    TIMESTAMPTZ  NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_chats_discipline_time
  ON public.chats (discipline_id, created_at DESC);

-- RLS para chats: public read, authenticated write
ALTER TABLE public.chats ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "chat_public_read" ON public.chats;
CREATE POLICY "chat_public_read"
  ON public.chats FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "chat_write_authenticated" ON public.chats;
